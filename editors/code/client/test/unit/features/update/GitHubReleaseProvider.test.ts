  ...
  describe("fetchLatestRelease", () => {
    it("returns ReleaseInfo when API returns valid response", async () => {
      const mockResponse = {
        tag_name: "v1.2.3",
        html_url:
          "https://github.com/GroovyLanguageServer/groovy-lsp/releases/tag/v1.2.3",
        published_at: "2024-01-15T10:00:00Z",
        assets: [
          {
            name: "groovy-lsp-1.2.3-linux-amd64.jar",
            browser_download_url: "https://example.com/download.jar",
          },
        ],
      };
    ...
    it("uses custom VersionChecker when provided", async () => {
      ...
      const mockResponse = {
        ...
        assets: [
          {
            name: "groovy-lsp-2.0.0.jar",
            browser_download_url: "https://example.com/v2.jar",
          },
        ],
      };
