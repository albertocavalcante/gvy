...
describe('MavenExecutionService', () => {
    ...
    describe('spawnMaven', () => {
        it('should detect HTTP blocker error in stderr and notify user', async () => {
            ...
            const promise = (service as any).spawnMaven('/cwd', [], consumerMock, token);

            // Simulate stderr with error
            const errorOutput = "Could not transfer artifact ... maven-default-http-blocker ...";
            ...
            assert.ok(
                vscodeMock.window.showErrorMessage.calledWithMatch("Maven Blocked HTTP Repository"),
