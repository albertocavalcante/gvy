...
describe("StatusBar", () => {
  ...
  beforeEach(() => {
    ...
    mockVscode = {
      window: {
        ...
        onDidChangeActiveTextEditor: sinon
          .stub()
    ...
    mockLanguageClient = {
      State,
      ProgressType: sinon.stub().callsFake(function () {
        return {};
      }),
    };
  ...
  describe("client state transitions", () => {
    ...
    it("should transition to stopped when client is Stopped", () => {
      ...
      assert.equal(
        statusBarItemStub.backgroundColor.id,
  ...
  describe("groovy/status notification handling", () => {
    ...
    it("should register groovy/status notification handler", () => {
      assert.isTrue(
        clientStub.onNotification.calledWith("groovy/status", sinon.match.func),
      );
    });
