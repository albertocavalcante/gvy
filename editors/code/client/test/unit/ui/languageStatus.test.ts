...
describe("LanguageStatus", () => {
  ...
  beforeEach(() => {
    ...
    languageStatusModule = proxyquire.noCallThru()(
      "../../../src/ui/languageStatus",
      {
        vscode: mockVscode,
  ...
  describe("createLanguageStatusManager", () => {
    ...
    it("should create server status item with correct id", () => {
      ...
      assert.isTrue(
        createLanguageStatusItemStub.calledWith(
          "groovy.server",
          sinon.match.array,
        ),
      );
  ...
  describe("LanguageStatusManager.updateServerStatus", () => {
    it("should update text for stopped state", () => {
      ...
      assert.equal(
        serverStatusItemStub.severity,
        mockVscode.LanguageStatusSeverity.Error,
      );
    });
    ...
    it("should update text for starting state", () => {
      ...
      assert.equal(
        serverStatusItemStub.severity,
        mockVscode.LanguageStatusSeverity.Information,
      );
    });
    ...
    it("should update text for degraded state", () => {
      ...
      assert.equal(
        serverStatusItemStub.severity,
        mockVscode.LanguageStatusSeverity.Warning,
      );
    });
  ...
  describe("LanguageStatusManager.updateJavaRuntime", () => {
    ...
    it("should show source in detail for jdk_manager", () => {
      const manager = languageStatusModule.createLanguageStatusManager();
      manager.updateJavaRuntime({
        path: "/home/user/.sdkman/candidates/java/21-tem",
        version: 21,
        source: "jdk_manager",
      });
