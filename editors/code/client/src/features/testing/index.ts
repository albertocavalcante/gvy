...
export function registerTestingFeatures(
  ...
  if (!testService) {
    logger.appendLine('[Testing] LSP client not ready. Testing features may be limited.');
  ...
  if (testService) {
    executionService = new LSPTestExecutionService(testService, logger, context.extensionPath);
  } else {
    ...
    executionService = new GradleExecutionService(logger, context.extensionPath);
  }
