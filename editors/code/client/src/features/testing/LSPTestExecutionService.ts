...
export class LSPTestExecutionService implements ITestExecutionService {
    ...
    async runTests(
        ...
        try {
            ...
            for (const item of testsToRun) {
                ...
                if (!uri) {
                    this.logger.appendLine(`[Error] Test item ${item.id} has no URI. Skipping.`);
                    continue;
                }
                ...
                const command = await this.testService.getTestCommand(uri, suiteName, testName);

                if (!command) {
                    this.logger.appendLine(`[Testing] LSP returned no command for ${item.id}. Build tool may not support test execution.`);
                    continue;
                }

                await this.executeCommand(command, consumer, token, isSuite ? 'gradle' : undefined);
    ...
    private async executeCommand(
        ...
        if (isGradle) {
            ...
            finalArgs = [
                '--init-script', this.initScriptPath,
                ...finalArgs,
            ...
            proc.on('close', (code) => {
                ...
                if (isMaven && code !== 0 && foundHttpBlocker) {
                    vscode.window.showErrorMessage(
                        ...
                        { modal: true, detail: 'Maven 3.8.1+ blocks insecure HTTP repositories.' },
                        'Open pom.xml'
                    ).then(s => {
                        if (s === 'Open pom.xml') {
                            const pom = path.join(cwd, 'pom.xml');
                            if (fs.existsSync(pom)) vscode.workspace.openTextDocument(pom).then(doc => vscode.window.showTextDocument(doc));
                        }
                    });
                }
