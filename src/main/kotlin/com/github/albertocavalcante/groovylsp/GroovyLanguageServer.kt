...
class GroovyLanguageServer :
    ...
    override fun initialize(params: InitializeParams): CompletableFuture<InitializeResult> {
        logInitializationStart(params)
        parseInitializationOptions(params)
        savedInitParams = params

        val capabilities = createServerCapabilities()
        val serverInfo = createServerInfo()

        return coroutineScope.future {
            try {
                initializeWorkspaceAsync(params)
    ...
    /**
     * Logs the start of initialization with client and workspace information.
     */
    private fun logInitializationStart(params: InitializeParams) {
    ...
    /**
     * Parses and validates initialization options from the client.
     */
    private fun parseInitializationOptions(params: InitializeParams) {
    ...
    /**
     * Creates server capabilities configuration.
     */
    private fun createServerCapabilities(): ServerCapabilities = ServerCapabilities().apply {
        configureBasicCapabilities()
        configureLanguageFeatures()
        configureAdvancedFeatures()
    }

    /**
     * Configures basic LSP capabilities.
     */
    private fun ServerCapabilities.configureBasicCapabilities() {
    ...
    /**
     * Configures language feature capabilities.
     */
    private fun ServerCapabilities.configureLanguageFeatures() {
    ...
    /**
     * Configures advanced LSP capabilities.
     */
    private fun ServerCapabilities.configureAdvancedFeatures() {
     ...
     */
    private fun createServerInfo(): ServerInfo = ServerInfo().apply {
        name = "Groovy Language Server"
        version = Version.current
    }

    /**
     * Initializes workspace asynchronously if workspace compilation is enabled.
     */
    private suspend fun initializeWorkspaceAsync(params: InitializeParams) {
        ...
        if (workspaceRoot != null && configuration.shouldUseWorkspaceCompilation()) {
            initializeWorkspaceWithDependencies(workspaceRoot)
        } else {
    ...
    /**
     * Initializes workspace with dependency resolution and compilation.
     */
    private suspend fun initializeWorkspaceWithDependencies(workspaceRoot: Path) {
        logger.info("Initializing workspace with dependencies first...")

        val dependencies = resolveDependencies(workspaceRoot)
        setupWorkspaceCompilation(workspaceRoot, dependencies)
        val compilationResult = compileWorkspace(workspaceRoot)
     ...
     */
    private fun resolveDependencies(workspaceRoot: Path): List<Path> {
        logger.info("Resolving dependencies...")
        return try {
            val resolver = SimpleDependencyResolver()
            val dependencies = resolver.resolveDependencies(workspaceRoot)
    ...
    /**
     * Sets up workspace compilation services.
     */
    private fun setupWorkspaceCompilation(workspaceRoot: Path, dependencies: List<Path>) {
    ...
    private fun logCompilationResult(result: WorkspaceCompilationService.CompilationResult) {
        if (result.isSuccess) {
            ...
            logger.warn(
                "Workspace compilation completed with errors: ${result.diagnostics.values.sumOf {
                    it.size
                }} diagnostics",
            )
        }
